...
try:
  ...
  # ~~~~ INSERT CODE ~~~~
  serverSocket = socket.socket(socket.AF_INET, socket.SOCK_STREAM) #Use the normal IPV4 web connectto the TCP
  # ~~~~ END CODE INSERT ~~~~
...
try:
  ...
  # ~~~~ INSERT CODE ~~~~
  serverSocket.bind((proxyHost, proxyPort)) #Bind a host and port
  # ~~~~ END CODE INSERT ~~~~
  print ('Port is bound')
except:
...
try:
  ...
  # ~~~~ INSERT CODE ~~~~
  serverSocket.listen(8) #Listen the socket
  # ~~~~ END CODE INSERT ~~~~
...
while True:
  ...
  try:
    # ~~~~ INSERT CODE ~~~~
    clientSocket, clientAddress = serverSocket.accept() #Accept from the client
    # ~~~~ END CODE INSERT ~~~~
  ...
  # ~~~~ INSERT CODE ~~~~
  message_bytes = clientSocket.recv(BUFFER_SIZE) #Store the info in clientsocket by HTTP GET 
  # ~~~~ END CODE INSERT ~~~~
  ...
  try:
    ...
    for info in cacheData :
      clientSocket.send(info.encode()) #Convert to the bytestream
    # ~~~~ END CODE INSERT ~~~~
    cacheFile.close()
    ...
    # ~~~~ INSERT CODE ~~~~
    originServerSocket = socket.socket(socket.AF_INET, socket.SOCK_STREAM) #Connet origin and use HTTP IPV4
    # ~~~~ END CODE INSERT ~~~~
    ...
    try:
      ...
      # ~~~~ INSERT CODE ~~~~
      originServerSocket.connect((address,80)) #Connect to the origin with localhost HTTP 80 and DNS 53
      # ~~~~ END CODE INSERT ~~~~
      ...
      # ~~~~ INSERT CODE ~~~~
      originServerRequest = f"GET {resource} HTTP/1.1" #GET method
      ...
      # ~~~~ INSERT CODE ~~~~
      originRes = originServerSocket.recv(BUFFER_SIZE) #GET the response from serversocket
      # ~~~~ END CODE INSERT ~~~~
